...
class FengWuUpscaler:
    ...
    def fengwu_upscale_forecast(self, data_dir, upscaled_model_fields, upscaled_derived_fields, get_date, fhr, ic, interp=True):
        print('upscaling', fhr)
        ...
        try:
            ...
            #    ds = xr.open_dataarray(os.path.join(data_dir, get_date, f'fengwu_pred_{str(fhr).zfill(3)}.nc')).isel(lat=slice(None, None, -1))
            #elif 'era5' in data_dir_lower:
            #    ds = xr.open_dataarray(os.path.join(data_dir, get_date, f'fengwu_pred_{str(fhr).zfill(3)}.nc')).isel(lat=slice(None, None, -1))
            #elif 'hresan' in data_dir_lower:
            #    ds = xr.open_dataarray(os.path.join(data_dir, get_date, f'fengwu_pred_{str(fhr).zfill(3)}.nc')).isel(lat=slice(None, None, -1))
            
            ds = xr.open_dataarray(os.path.join(data_dir, get_date, ic, 'fengwu_forecast_data', f'fengwu_{ic}_pred_{str(fhr).zfill(3)}.nc')).isel(lat=slice(None, None, -1))
