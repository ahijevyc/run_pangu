...
class GraphcastUpscaler:
    ...
    def graphcast_upscale_forecast(self, data_dir, upscaled_model_fields, upscaled_derived_fields, get_date, fhr, interp=True):
        ...
        try:
            ds = xr.open_dataset(os.path.join(data_dir, get_date, f'gc_oper_gfs_pred_{str(fhr).zfill(3)}.nc')).isel(time=0, lat=slice(None,None,-1), level=slice(None,None,-1))
            
            ds.coords['lon'] = (ds.coords['lon'] + 180) % 360 - 180
            ds = ds.sortby(ds.lon).compute()
