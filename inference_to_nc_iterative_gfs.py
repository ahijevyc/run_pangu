...
def run_inference(date, input_dir, output_dir, ort_session_24, ort_session_6):
    ...
    all_exists = True
    for i in range(40):
        nc_file = os.path.join(date_output_dir, f'pangu_gfs_pred_{str((i+1)*6).zfill(3)}.nc')
    ...
    for i in range(40):
        ...
        # Save as netCDF
        output_filename = os.path.join(date_output_dir, f'pangu_gfs_pred_{str((i+1)*6).zfill(3)}.nc')
