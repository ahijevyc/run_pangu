...
from utils.util_fengwu_infer import FengWuUpscaler
import argparse
...
fengwuupscaler = FengWuUpscaler(basemap_products=basemap_products)

def upscale_fengwu_forecast(fcst_hr, input_date, ic):
    return fengwuupscaler.fengwu_upscale_forecast(data_dir, 
...
for input_date in input_dates:
    ...
    output_filename = f'{output_dir}/{dt_input_date.strftime("%Y%m%d%H")}_FengWu_{args.ic}_upscaled.nc'
    ...
    upscale_forecast_partial = partial(upscale_fengwu_forecast, input_date=input_date, ic=args.ic)
