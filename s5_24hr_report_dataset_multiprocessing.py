...
def process_single_date(thisdate, config, geo, ic):
    ...
    for i, dtdir in enumerate(dataset_folder):
        ...
        fname = f'{nc_rootdir}/{thisdate.strftime("%Y%m%d%H")}/{ic}/{thisdate.strftime("%Y%m%d%H")}_Pangu_{ic}_upscaled.nc'
        print(fname)
    ...
    if len(fnames) > 0:
        ...
        outfile = os.path.join(write_dir,f'grid_data_pangu_{ic}_avg_d{str(start_fcst_day).zfill(2)}_24hr_{yyyymmddhh}-0000.par')
